...
card_fraud %>% 
  select(is_fraud, amt, city_pop, distance_km, )%>% #selecting the required variables
...
for (var in variables) {
  qqnorm(card_fraud[[var]], main = paste("Q-Q Plot of", var))
  qqline(card_fraud[[var]], col = "red")
}
```
...
subset_df <- card_fraud[, variables]
...
- Looking at the variable importance of the current best model
...
# Model Fit - 2nd Iteration

## Update the tidymodels `recipe`
...
## Bundle new recipe and model with `workflows`

```{r, define_new_workflows}
...
```{r}
set.seed(123)
cv_folds_full <- vfold_cv(data = card_fraud_train, 
                          v = 10, 
                          strata = is_fraud)
cv_folds_full 
```
...
log_res_full %>%  collect_metrics(summarize = TRUE)
```
