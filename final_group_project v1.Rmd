...
```{r}
colnames(card_fraud)
fraud_recipe <- recipe(is_fraud ~ category + amt + city_pop + job + distance_miles + distance_km, data = card_fraud) %>%
  step_rm(trans_year, city, state, lat, long, dob, merch_lat, merch_long)

# Prepare the recipe
fraud_recipe_prep <- prep(fraud_recipe)

# Apply the prepared recipe to the dataset
fraud_data <- bake(fraud_recipe_prep, new_data = card_fraud)
...
XXXXXXXX full set for best model!!!
